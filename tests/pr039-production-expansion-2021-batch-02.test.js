...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "paperlens-pr039-"));
...
if (!before.papers.some((paper) => paper.id === "0478-2021-MJ-13-QP")) {
  const report = publishProductionExpansion({
    rootDir,
    storePath,
    batchId: "PR039-0478-2021-MJ-BATCH-02",
    syllabus: "0478",
    year: 2021,
    session: "M/J",
    pairs
...
if (production.expansionBatches.some((batch) => batch.id === "PR039-0478-2021-MJ-BATCH-02")) {
  assert.deepEqual(
    production.papers.filter((paper) => paper.syllabus === "0478" && paper.year === 2021 && paper.session === "M/J" && ["13", "21"].includes(paper.component)).map((paper) => paper.id).sort(),
    ["0478-2021-MJ-13-MS", "0478-2021-MJ-13-QP", "0478-2021-MJ-21-MS", "0478-2021-MJ-21-QP"]
  );
}
