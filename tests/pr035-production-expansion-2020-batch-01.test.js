...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "paperlens-pr035-"));
...
if (!before.papers.some((paper) => paper.id === "0478-2020-MJ-11-QP")) {
  const report = publishProductionExpansion({
    rootDir,
    storePath,
    batchId: "PR035-0478-2020-MJ-BATCH-01",
    syllabus: "0478",
    year: 2020,
    session: "M/J",
    pairs
...
if (production.expansionBatches.some((batch) => batch.id === "PR035-0478-2020-MJ-BATCH-01")) {
  assert.deepEqual(
    production.papers.filter((paper) => paper.syllabus === "0478" && paper.year === 2020 && paper.session === "M/J" && ["11", "12"].includes(paper.component)).map((paper) => paper.id).sort(),
    ["0478-2020-MJ-11-MS", "0478-2020-MJ-11-QP", "0478-2020-MJ-12-MS", "0478-2020-MJ-12-QP"]
  );
}
